...
pub fn resolve_and_normalize(relative_path: &Path, base_directory: &Path) -> Result<PathBuf, io::Error> {
...
pub fn find_file_by_name(root: &Path, target_name: &str) -> io::Result<Vec<PathBuf>> {
    ...
    if !root.is_dir() {
        return Err(io::Error::new(io::ErrorKind::InvalidInput, "Root path is not a directory"));
    }
...
pub fn find_file_by_name_case_insensitive(root: &Path, target_name: &str) -> io::Result<Vec<PathBuf>> {
    let lower_target = target_name.to_lowercase();

    find_file_by_name_with_filter(root, |name| {
        name.to_lowercase() == lower_target
    })
}
